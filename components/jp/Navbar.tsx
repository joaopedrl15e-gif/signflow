...
import { Menu, X, ArrowUpRight, Instagram, Mail, Github } from 'lucide-react';
...
export const Navbar: React.FC = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      // Check scroll depth for glass effect
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 pt-3.5 pb-2">
        <div
          className={`rounded-2xl px-5 py-3 flex items-center justify-between transition-all duration-300 ${
            isScrolled
              ? 'bg-[#060317]/85 backdrop-blur-xl border border-purple-500/25 shadow-2xl shadow-purple-950/50'
              : 'bg-transparent border border-white/[0.04]'
          }`}
        >
          {/* Logo "JP" with purple and blue gradient */}
          <button
            onClick={() => scrollToSection('inicio')}
            className="flex items-center gap-3 group text-left cursor-pointer"
          >
            <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-purple-600 via-blue-600 to-cyan-400 p-0.5 shadow-lg shadow-purple-500/30 group-hover:scale-105 transition-transform duration-200">
              <div className="w-full h-full bg-[#030014] rounded-[10px] flex items-center justify-center font-mono font-black text-sm text-transparent bg-clip-text bg-gradient-to-tr from-white via-purple-200 to-cyan-300">
                JP
              </div>
            </div>
          ...
          <nav className="hidden md:flex items-center gap-1 bg-[#0b0526]/60 px-4 py-1.5 rounded-full border border-purple-500/20 shadow-inner">
            {NAV_LINKS.map((link) => {
              ...
              return (
                ...
                >
                  ...
                  {isActive && (
                    <span className="absolute bottom-0 left-2 right-2 h-[2px] bg-gradient-to-r from-purple-500 via-blue-500 to-cyan-400 rounded-full shadow-sm animate-in fade-in zoom-in-95 duration-200" />
                  )}
                </button>
              );
            })}
          ...
          <div className="hidden md:flex items-center gap-3">
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 text-white font-bold text-xs shadow-lg shadow-purple-600/30 transition-all hover:scale-105 active:scale-95 cursor-pointer"
            >
      ...
      {mobileMenuOpen && (
        <div className="md:hidden fixed inset-0 z-40 bg-[#030014]/95 backdrop-blur-2xl p-6 pt-24 animate-in fade-in duration-200 flex flex-col justify-between">
          ...
          <div className="pt-6 border-t border-purple-500/20 flex flex-col items-center gap-3">
            <button
              onClick={() => scrollToSection('contato')}
              className="w-full py-3.5 rounded-xl bg-gradient-to-r from-purple-600 to-blue-600 text-white font-bold text-sm shadow-xl"
            >
              Entrar em contato
            </button>
