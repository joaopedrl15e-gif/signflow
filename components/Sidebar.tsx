...
export const Sidebar: React.FC = () => {
  ...
  const usageCurrent = subData?.usage?.current || 0;
  const usageMax = subData?.usage?.max || 3;

  return (
    <aside className="w-64 bg-white text-slate-800 flex flex-col border-r border-slate-200/90 min-h-screen shrink-0 shadow-sm relative z-30 justify-between">
      <div>
        {/* Brand Header */}
      ...
      <div className="p-3">
        ...
        <div className="px-3 py-2 rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-between">
          <div className="flex items-center gap-2 overflow-hidden">
            ...
            <span className="text-xs font-bold text-slate-800 truncate">{user?.name || 'Minha Conta'}</span>
          </div>

          <button
            onClick={handleLogout}
            title="Sair da Conta"
            className="p-1 rounded-lg text-slate-400 hover:text-rose-600 hover:bg-white transition-colors shrink-0"
          >
